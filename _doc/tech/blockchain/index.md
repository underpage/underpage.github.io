---
order: 2
title: Blockchain
description: 블록체인 (인덱스)
summary:
keywords:
- blockchain
- smart contract
- decentralized app
- ethereum
tags:
- index
- blockchain
- tech
created_time: 2025-04-04 01:19
modified_time: 2025-04-04 01:19
---

# Blockchain
: 분산된 네트워크에서 암호화 기술을 이용해 데이터를 안전하게 저장하고 거래하는 기술 집합

- 블록체인 구성 요소
  - [블록체인 구조](./blockchain-structure.md)
  - [암호학](../../security/security-technology/cryptography.md)
  - [합의 메커니즘](./consensus-mechanism.md)
  - [블록체인 보안](./blockchain-security.md)
- [블록체인 네트워크](./blockchain-network.md)
  - 퍼블릭 블록체인
  - 프라이빗 블록체인
  - 컨소시엄 블록체인
  - 하이브리드 블록체인
- 블록체인 기술
  - 스마트 컨트랙트
- [블록체인 경제 시스템](./blockchain-economy.md)
  - 디지털 자산
  - 토큰 이코노미 
- [블록체인 확장성](./blockchain-scalability.md)
- [블록체인 상호운용성](./blockchain-interoperability.md)
- [블록체인 플랫폼](./blockchain-platform/index.md)
  - 비트코인
  - 이더리움
  - 솔라나
  - 폴카닷
  - 카르다노
- [블록체인 응용](./blockchain-application/index.md)
  - 탈중앙화 애플리케이션 (DApp)
  - 탈중앙화 금융 (DeFi)
  - 탈중앙화 신원 (DID)
  - 탈중앙화 자율조직 (DAO)
  - 탈중앙화 스토리지
- [블록체인 산업 응용](./blockchain-industries.md)
  - 금융 서비스
  - 공급망 관리
  - 의료 데이터 관리
  - 정부 및 공공 서비스
  - 스마트 계약과 법률


**주요 특징**
- 탈중앙화
- 변조 불가능성
- 투명성
- 안전성
- 익명성
- 가명성