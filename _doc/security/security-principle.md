...
modified_time: 2025-05-29 23:42
---

# Security Principle
: 보안의 핵심 원칙 및 근본적인 목표는 자산의 기밀성, 무결성, 가용성을 지키는 것  
...
: 도난이나 도청으로부터 안전하게 보호되도록 암호화, 접근 제어, 권한 관리 등의 기술을 사용함  

**무결성** (Integrity)  
: 데이터가 무단으로 변경되지 않고 인증된 방식으로 변경할 수 있어야 함  
: 인증되지 않은 방식으로 데이터를 변경, 위조, 전달하는 것을 방지해야 함  
: 권한있는 개체가 데이터를 생성, 수정, 삭제한다면 이를 추적하고 확인할 수 있어야 함  
: 데이터의 무결성을 확인하기 위해 해시 함수, 디지털 서명, 체크섬 등 암호화 기술을 사용함

**가용성** (Availability)  
: 권한있는 개체는 언제든지 데이터나 시스템에 접근할 수 있어야 함  
