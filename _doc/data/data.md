...
: 이를 극복하기 위해 데이터를 체계적으로 구조화하며 데이터의 패턴을 파악하기도 함  
...
**구조화 발전 과정**
```
물리적 분류 > 체계적 분류법 도입 (카탈로그, 색인) > 기계적 처리 (파일 시스템) > 
디지털 구조화 (데이터베이스) > 자동화된 구조화 (데이터 레이크) > 지능형 구조화 (자동 분류) 
```
...
: 데이터만으로 지혜가 만들어지기 어려우며 단계별 인간의 개입과 해석이 들어가야 활용도가 높아짐
...
미래에는 자율적으로 데이터를 생성하고 예측을 분석하고 지능형 의사 결정 수행을 목표로 함
```
