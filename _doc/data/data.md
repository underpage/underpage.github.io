...
category: Data
title: 데이터와 정보
summary: 데이터 개념·분류·분석·구조화, DIKW 위계 모형, 정보·지식·지혜의 정의와 유형
keywords:
- data
- dikw
- information
...
: 데이터 그 자체로는 의미가 부족하며 데이터 분석을 통해 유용한 정보로 가치화됨  

**데이터 분석**  
: 데이터를 정제, 변환, 해석하여 유용한 정보를 도출하는 과정  
: 이를 통해 데이터에 내재된 패턴, 관계, 추세 등을 발견하고 의미를 부여함  
: 데이터를 분석하기 위해서는 데이터를 체계적으로 구조화해야 함  

```
데이터를 분석하는 궁극적 목적은 데이터에서 가치를 창출하는 것으로
데이터가 실질적 가치를 갖기 위해서는 더 높은 단계의 변환이 필요함 (DIKW)
```

**데이터 구조화**  
: 인간의 뇌는 대량의 정보를 동시에 처리하는데 한계가 존재함  
: 이를 극복하기 위해 데이터를 체계적으로 구조화하여 데이터의 관계와 패턴을 명확히 파악할 수 있음  
: 데이터를 구조화하기 위해 분류, 정렬, 계층화, 그룹화, 태깅 등의 방법을 사용함  

방법 | 설명 | 효과
---|---|---
분류 | 정의된 기준에 따라 범주화 | 의미 명확성 및 일관성
정렬 | 특정 속성에 따라 순서화 | 검색/탐색 효율성
계층화 | 상하위 관계에 따라 트리 구조로 표현 | 관계 명확성
그룹화 | 유사한 항목끼리 묶음 | 집합적 특성 파악
태깅 | 데이터와 관련된 추가 정보(메타데이터) 부여 | 검색 범위 확장
...
## DIKW
: 러셀 애코프가 1989년 논문에서 제안한 가치 위계 모형  
: 데이터부터 정보, 지식, 지혜를 만들어 가는 과정을 표현한 피라미드  
: 데이터만으로는 지혜가 만들어지기 어려우며 단계별 인간의 개입과 해석이 더해져야 가치로 전환됨  
...
3. 이를 통해 정보들 간의 관계와 패턴을 파악해 예측 모델 등의 지식을 구축하고
4. 생성된 지식과 경험을 통해 최적의 의사 결정을 수행함


데이터 엔지니어링은 DIKW의 각 단계 전환이 원활하게 이루어질 수 있도록 데이터 흐름을 관리함

1. 다양한 소스에서 데이터를 수집하고 저장 시스템에 적재하고
2. ETL/ELT 파이프라인을 통해 데이터를 정제·변환하여 사용 가능한 형태로 가공하고
3. 처리된 데이터를 분석 시스템이나 소비 서비스에 안정적으로 제공함


```
과거 컴퓨터 시스템은 대량의 데이터를 수집하고 처리하였으나
현재 컴퓨터 시스템은 실시간 데이터를 수집 및 처리하고 지식을 추출하는 단계로 발전함
미래에는 자율적으로 데이터를 생성하고 상황을 예측하여 지능형 의사 결정 수행을 목표로 함
```
...
: 정보의 패턴이나 관계를 발견하고, 이질적인 정보들을 연결해 새로운 지식을 생성함  
