...
modified_time: 2025-03-04 22:35
---
...
- [기본 타입](#primitive-type) 데이터가 값으로 표현
- [참조 타입](#reference-type) 데이터가 구조로 표현
...
: 자바는 데이터의 특성에 따라 기본 타입을 8가지로 분류해 제공함  
...
: 래퍼 클래스는 [불변 객체](./java-object-characteristics.md#immutable-object)로 내부 값을 변경할 수 없음  

```java
Integer num = 0;

// 새로운 Integer 객체가 생성되고 num 변수가 이를 참조함
num = 100;
```
...
### char
: 16비트 크기의 문자 데이터 타입으로 부호가 없으며 0 ~ 65535 범위의 한 개 문자를 저장함  
